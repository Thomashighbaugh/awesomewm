...
local init = function()
    ...
    capi.client.connect_signal("manage", function(c)
        if consoles[c.pid] ~= nil then consoles[c.pid]:show(c) end
    end)
    capi.client.connect_signal("unmanage", function(c)
        if consoles[c.pid] ~= nil then consoles[c.pid]:hide(c) end
    end)
end
...
function QuakeConsole:findClient()
    ...
    for c in awful.client.iterate(function(c) return c.pid == self.pid end) do
        return c
    end
end
...
function QuakeConsole:show(client)
    ...
    if not client then
        ...
        self.pid = awful.util.spawn(self.terminal .. " " ..
...
function QuakeConsole:new(config)
    ...
    config.terminal = "kitty" or "alacritty" or "tilix" -- application to spawn
...
setmetatable(QuakeConsole,
             {__call = function(_, ...) return QuakeConsole:new(...) end})
