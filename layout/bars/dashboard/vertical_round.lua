...
local small_font = beautiful.font .. '12'
local bar_background = beautiful.xcolor0 .. 'cc'
...
function create_box(widget)
    ...
    temp_wid:connect_signal("mouse::enter", function()
        temp_wid:get_children_by_id("border")[1].bg =
            dashboard_border_color_focus
    end)
    temp_wid:connect_signal("mouse::leave", function()
        temp_wid:get_children_by_id("border")[1].bg =
            dashboard_border_color_normal
    end)
    return temp_wid
end
            ...
            {
                {
                    ...
                    forced_width = (dashboard_width * 0.85 - dpi(10) - 4 *
                        dashboard_border_width) / 2,
                    widget = wibox.container.background
                },
                margins = dashboard_border_width,
...
fancy_date_widget.markup = helpers.colorize_text(fancy_date_widget.text,
                                                 beautiful.xcolor12)
...
function create_blank_bar(fg_color1, fg_color2, bg_color)
    local fg_color = {
        ...
        stops = {
            {0.00, fg_color1}, {0.10, fg_color1}, {0.55, fg_color2},
            {0.85, fg_color2}
        }
    }
...
local cpu_bar = create_blank_bar(beautiful.xcolor1, beautiful.xcolor9,
                                 bar_background)

awesome.connect_signal("signal::cpu", function(percentage)
...
local ram_bar = create_blank_bar(beautiful.xcolor2, beautiful.xcolor10,
                                 bar_background)
...
local disk_bar = create_blank_bar(beautiful.xcolor3, beautiful.xcolor11,
                                  bar_background)
...
local vol_bar = create_blank_bar(beautiful.xcolor4, beautiful.xcolor12,
                                 bar_background)
...
local bri_bar = create_blank_bar(beautiful.xcolor5, beautiful.xcolor13,
                                 bar_background)
...
awesome.connect_signal("signal::newsboat", function(unread_items)
    newsboat_heading.markup = helpers.colorize_text(tostring(unread_items),
                                                    newsboat_fg)
end)
...
awesome.connect_signal("signal::temp", function(temperatur)
    temp_heading.markup = helpers.colorize_text(tostring(temperatur) .. "°C",
                                                temp_fg)
end)
...
local update_fortune = function()
    awful.spawn.easy_async_with_shell(fortune_command, function(out)
        ...
        out = out:gsub('^%s*(.-)%s*$', '%1')
        fortune.markup =
            "<i>" .. helpers.colorize_text(out, beautiful.xcolor3) .. "</i>"
    end)
end
...
local fortune_widget = wibox.widget {
    {nil, fortune, layout = wibox.layout.align.horizontal},
    margins = 4,
    color = "#00000000",
    widget = wibox.container.margin
}
...
awesome.connect_signal("signal::weather", function(temp, wind, emoji)
    weather_heading.markup = helpers.colorize_text(tostring(temp) .. "°C",
                                                   weather_fg)
    weather_info.markup = helpers.colorize_text(
                              " <i>Temperature\n<b>IRL</b> (" .. tostring(wind) ..
                                  "km/h wind)</i>", weather_fg)
end)
...
local piechart = wibox.widget {
    data_list = {{'L1', 100}, {'L2', 200}, {'L3', 300}},
    border_width = 1,
    border_color = beautiful.xfg,
    colors = {beautiful.xcolor2, beautiful.xcolor1, beautiful.xcolor5},
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mybar:setup{
        ...
        fancy_time_box,
        performance_box,
        {news_box, temp_box, layout = wibox.layout.fixed.horizontal},
        -- acititywatch_box,
        user_box,
        {fortune_box, layout = wibox.layout.fixed.horizontal},
        
    }

    s.mybar:connect_signal("mouse::leave",
                           function() awesome.emit_signal("toggle::dash") end)
