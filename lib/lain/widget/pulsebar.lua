...
local function factory(args)
    ...
    pulsebar.cmd = args.cmd or "pacmd list-" .. pulsebar.devicetype ..
            ...
            if volu ~= pulsebar._current_level or mute ~= pulsebar._mute then
                ...
                if pulsebar._current_level == 0 or mute == "yes" then
                    ...
                    pulsebar.tooltip:set_text(
                        string.format("%s %s: %s", pulsebar.devicetype,
                                      pulsebar.device, volu))
    ...
    function pulsebar.notify()
        pulsebar.update(function()
            ...
            preset.title = string.format("%s %s - %s%%", pulsebar.devicetype,
                                         pulsebar.device,
            ...
            if not pulsebar.notification then
                ...
                naughty.replace_text(pulsebar.notification, preset.title,
                                     preset.text)
            end
        end)
    end

    helpers.newtimer(string.format("pulsebar-%s-%s", pulsebar.devicetype,
                                   pulsebar.device), timeout, pulsebar.update)
