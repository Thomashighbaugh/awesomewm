...
local function factory(args)
    ...
    local cal = {
        ...
        week_number_format = args.week_number_format or args.week_number ==
            "left" and "%3d | " or "| %-3d",
    ...
    function cal.get_week_number(m, st_day, x)
        return string.format(cal.week_number_format, os.date("%V", m) +
                                 (x ~= 0 and floor((x + st_day) / 7) - 1 or 0))
    end

    function cal.sum_week_days(x, y) return (x + y) % 7 end

    function cal.build(month, year)
        local current_month, current_year = tonumber(os.date("%m")),
                                            tonumber(os.date("%Y"))
        local is_current_month = (not month or not year) or
                                     (month == current_month and year ==
                                         current_year)
        ...
        local d = os.date("*t", t)
        local mth_days, st_day, this_month = d.day, (d.wday - d.day -
                                                 cal.week_start + 1) % 7,
                                             os.date("%B %Y", t)
        local notifytable = {
            [1] = string.format("%s%s\n", string.rep(" ", floor(
        ...
        for x = 0, 6 do
            notifytable[#notifytable + 1] =
                os.date("%a", os.time {
                    year = 2006,
                    month = 1,
                    day = x + cal.week_start
                }):sub(1, 3) .. " "
        end
        notifytable[#notifytable] = string.format("%s\n%s",
                                                  notifytable[#notifytable]:sub(
                                                      1, -2),
                                                  string.rep(" ", st_day * 4))
        local strx
        for x = 1, mth_days do
            ...
            if x == today then
                ...
                strx = markup.bold(markup.color(cal.notification_preset.bg,
                                                cal.notification_preset.fg, x) ..
                                       " ")
            end
            strx = string.format("%s%s", string.rep(" ", 3 - tostring(x):len()),
                                 strx)
            notifytable[#notifytable + 1] =
                string.format("%-4s%s", strx, (x + st_day) % 7 == 0 and x ~=
                                  mth_days and "\n" or "")
        end
        ...
        if cal.week_number ~= "none" then
            ...
            local head_prepend = string.rep(" ", tostring(
                                                string.format(
                                                    cal.week_number_format, 0)):len())
            ...
            if cal.week_number == "left" then
                ...
                notifytable[8] = notifytable[8]:gsub("\n", "\n" ..
                                                         cal.get_week_number(m,
                                                                             st_day,
                                                                             0)) -- first week of the month

                for x = 10, #notifytable do
                    if cal.sum_week_days(st_day, x) == 2 then
                        notifytable[x] =
                            cal.get_week_number(m, st_day, x) .. notifytable[x]
                    end
                end
            elseif cal.week_number == "right" then
                ...
                notifytable[#notifytable] =
                    notifytable[#notifytable] .. string.rep(" ", 4 * end_days) ..
    ...
    function cal.show(seconds, month, year, scr)
        ...
        cal.notification = naughty.notify {
            ...
            icon = cal.icon,
            timeout = type(seconds) == "number" and seconds or
                cal.notification_preset.timeout or 5
        }
    end
    ...
    function cal.attach(widget)
        ...
        widget:buttons(awful.util.table.join(awful.button({}, 1, cal.prev),
