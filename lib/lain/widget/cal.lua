...
local function factory(args)
	...
	local cal = {
		...
		notification_preset = args.notification_preset or { font = "Monospace 10", fg = "#FFFFFF", bg = "#000000" },
	}

	function cal.get_week_number(m, st_day, x)
		return string.format(
			cal.week_number_format,
			os.date("%V", m) + (x ~= 0 and floor((x + st_day) / 7) - 1 or 0)
		)
	end
