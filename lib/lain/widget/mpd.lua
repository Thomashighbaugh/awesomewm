...
local function factory(args)
    ...
    local password = (args.password and #args.password > 0 and
    ...
    local echo = string.format("printf \"%sstatus\\ncurrentsong\\nclose\\n\"",
                               password)
    local cmd = string.format("%s | curl --connect-timeout 1 -fsm 3 %s", echo,
                              mpdh)
    ...
    function mpd.update()
        helpers.async({shell, "-c", cmd}, function(f)
            ...
            if mpd_now.state == "play" then
                if notify == "on" and mpd_now.title ~=
                    helpers.get_map("current mpd track") then
