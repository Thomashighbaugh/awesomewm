...
local function format_progress(elapsed, duration)
	...
	if dm < 600 then
		return ("%d:%02d:%02d"):format(eh, em, es), ("%d:%02d:%02d"):format(dh, dm, ds)
	else
		return ("%02d:%02d:%02d"):format(eh, em, es), ("%02d:%02d:%02d"):format(dh, dm, ds)
	end
end
-- }}}
...
function mpd_all.async(format, warg, callback)
	...
	local query = ("printf 'password %s\nstatus\ncurrentsong\nclose\n'"):format(warg and (warg.password or warg[1]) or "\"\"")
	...
	spawn.with_line_callback_with_shell(query .. "|" .. connect, {
		stdout = function(line)
			for k, v in line:gmatch("([%w]+):[%s](.*)$") do
				local key = "{" .. k .. "}"
				if k == "volume" or k == "bitrate" or k == "elapsed" or k == "duration" then
				...
				elseif
					k == "Artist"
					or k == "Title" -- k == "Name" or k == "file" or
					or k == "Album"
					or k == "Genre"
				then
					mpd_state[key] = v
