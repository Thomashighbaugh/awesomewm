...
local function parse(stdout, stderr, exitreason, exitcode)
	...
	_weather["{city}"] = stdout:match("^(.+)%,.*%([%u]+%)") or _weather["{city}"]
	_weather["{wind}"] = stdout:match("Wind:[%s][%a]+[%s][%a]+[%s](.+)[%s]at.+$") or _weather["{wind}"]
	_weather["{windmph}"] = stdout:match("Wind:[%s].+[%s]at[%s]([%d]+)[%s]MPH") or _weather["{windmph}"]
	_weather["{sky}"] = stdout:match("Sky[%s]conditions:[%s](.-)[%c]") or _weather["{sky}"]
	_weather["{weather}"] = stdout:match("Weather:[%s](.-)[%c]") or _weather["{weather}"]
	_weather["{tempf}"] = stdout:match("Temperature:[%s]([%-]?[%d%.]+).*[%c]") or _weather["{tempf}"]
	_weather["{dewf}"] = stdout:match("Dew[%s]Point:[%s]([%-]?[%d%.]+).*[%c]") or _weather["{dewf}"]
	_weather["{humid}"] = stdout:match("Relative[%s]Humidity:[%s]([%d]+)%%") or _weather["{humid}"]
	_weather["{press}"] = stdout:match("Pressure[%s].+%((.+)[%s]hPa%)") or _weather["{press}"]

	local year, month, day, hour, min = stdout:match("(%d%d%d%d).(%d%d).(%d%d) (%d%d)(%d%d) UTC")
	if year ~= nil then
		local utctable = {
			year = year,
			month = month,
