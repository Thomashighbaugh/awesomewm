...
local function parse(stdout, stderr, exitreason, exitcode)
    ...
    stdout:match "^(.+)%,.*%([%u]+%)" or _weather["{city}"]
    _weather["{wind}"] = -- Wind direction and degrees if available
    stdout:match "Wind:[%s][%a]+[%s][%a]+[%s](.+)[%s]at.+$" or
        _weather["{wind}"]
    _weather["{windmph}"] = -- Wind speed in MPH if available
    stdout:match "Wind:[%s].+[%s]at[%s]([%d]+)[%s]MPH" or _weather["{windmph}"]
    _weather["{sky}"] = -- Sky conditions if available
    stdout:match "Sky[%s]conditions:[%s](.-)[%c]" or _weather["{sky}"]
    _weather["{weather}"] = -- Weather conditions if available
    stdout:match "Weather:[%s](.-)[%c]" or _weather["{weather}"]
    _weather["{tempf}"] = -- Temperature in fahrenheit
    stdout:match "Temperature:[%s]([%-]?[%d%.]+).*[%c]" or _weather["{tempf}"]
    _weather["{dewf}"] = -- Dew Point in fahrenheit
    stdout:match "Dew[%s]Point:[%s]([%-]?[%d%.]+).*[%c]" or _weather["{dewf}"]
    _weather["{humid}"] = -- Relative humidity in percent
    stdout:match "Relative[%s]Humidity:[%s]([%d]+)%%" or _weather["{humid}"]
    _weather["{press}"] = -- Pressure in hPa
    stdout:match "Pressure[%s].+%((.+)[%s]hPa%)" or _weather["{press}"]
...
function weather_all.async(format, warg, callback)
    ...
    local url = string.format(
                    ...
                    warg)
    spawn.easy_async("curl -fs " .. url, function(...) callback(parse(...)) end)
end
-- }}}
