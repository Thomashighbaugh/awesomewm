...
local function getBuilderStatus(b)
	...
	if b:getLastSuccessful() ~= 0 then
		local success_build_nr_str = "<span color=\"green\">" .. b:getLastSuccessful() .. "</span>"
	else
		local success_build_nr_str = "-"
	end

	local current_build_color = bc[b:getCurrentStatus()]
	local current_build_nr_str = "<span color=\"" .. current_build_color .. "\">" .. b:getCurrent() .. "</span>"
...
setmetatable(buildbot_all, { __call = function(_, ...)
	return worker(...)
end })
