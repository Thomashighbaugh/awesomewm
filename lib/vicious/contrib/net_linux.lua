...
local function worker(format, tignorelist)
    ...
    if not tignorelist then tignorelist = {"lo", "wmaster0"} end
    for k, i in pairs(tignorelist) do tignore[i] = true end
    ...
    for line in io.lines("/proc/net/dev") do
        ...
        if name ~= nil then
            ...
            if nets[name] == nil then
                ...
                local interval = os.time() - nets[name].time > 0 and os.time() -
                                     nets[name].time or 1
                nets[name].time = os.time()
    ...
    if nets["total"] == nil then
        ...
        local interval = os.time() - nets["total"].time > 0 and os.time() -
                             nets["total"].time or 1
        nets["total"].time = os.time()
