...
local function worker(format, tignorelist)
	...
	for line in io.lines("/proc/net/dev") do
		...
		if name ~= nil then
			...
			if nets[name] == nil then
				...
				local interval = os.time() - nets[name].time > 0 and os.time() - nets[name].time or 1
				nets[name].time = os.time()
				...
				if sysnet.carrier then
					local ccarrier = tonumber(sysnet.carrier)
	...
	if nets["total"] == nil then
		...
		local interval = os.time() - nets["total"].time > 0 and os.time() - nets["total"].time or 1
		nets["total"].time = os.time()
