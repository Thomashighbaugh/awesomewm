...
local function worker(format, warg)
	...
	_wdata["{city}"] = string.match(ws, "\"name\":\"([%a%s%-]+)\"") or _wdata["{city}"]
	_wdata["{wind deg}"] = string.match(ws, "\"deg\":([%d]+)") or _wdata["{wind deg}"]
	_wdata["{wind mps}"] = string.match(ws, "\"speed\":([%d%.]+)") or _wdata["{wind mps}"]
	_wdata["{sky}"] = string.match(ws, "\"main\":\"([%a]+)\"") or _wdata["{sky}"]
	_wdata["{weather}"] = string.match(ws, "\"description\":\"([%a%s]+)\"") or _wdata["{weather}"]
	_wdata["{temp c}"] = string.match(ws, "\"temp\":([%-]?[%d%.]+)") or _wdata["{temp c}"]
	_wdata["{humid}"] = string.match(ws, "\"humidity\":([%d]+)") or _wdata["{humid}"]
	_wdata["{press}"] = string.match(ws, "\"pressure\":([%d%.]+)") or _wdata["{press}"]

	-- Wind speed in km/h
	if _wdata["{wind mps}"] ~= "N/A" then
		_wdata["{wind mps}"] = math.floor(tonumber(_wdata["{wind mps}"]) + 0.5)
		_wdata["{wind kmh}"] = math.ceil(_wdata["{wind mps}"] * 3.6)
	end -- Temperature in °C
	if _wdata["{temp c}"] ~= "N/A" then
		_wdata["{temp c}"] = math.floor(tonumber(_wdata["{temp c}"]) + 0.5)
	...
	if _wdata["{wind deg}"] ~= "N/A" then
		...
		if (_wdata["{wind deg}"] / 45) % 1 == 0 then
			...
			_wdata["{wind aim}"] = _wdirs[math.ceil(_wdata["{wind deg}"] / 45) + 1]
