...
local session = require("lib.poppin.session")
...
function restore()
    ...
    for _, app in pairs(apps) do setProperties(app) end
end
...
function setProperties(app)
    ...
    awful.rules
        .execute(app.client, {width = props.width, height = props.height})
    awful.rules.execute(app.client, props)
    app.client:connect_signal("unfocus",
                              function() app.client.minimized = true end)
end

function toggle(name)
    local c = apps[name].client
    c.minimized = not c.minimized
    if not c.minimized then client.focus = c end
end
