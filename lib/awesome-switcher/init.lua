...
_M.settings = {
    preview_box = true,
    preview_box_bg = "#292d35aa",
    preview_box_border = "#292d3500",
    preview_box_fps = 30,
    preview_box_delay = 150,
    preview_box_title_font = {"sans", "italic", "normal"},
    preview_box_title_font_size_factor = 1.25,
        ...
        for ci = 1, #clients do
            for ti = 1, #_M.altTabTable do
                if _M.altTabTable[ti].client == clients[ci] then
                    _M.altTabTable[ti].client.opacity =
                        _M.altTabTable[ti].opacity
                    _M.altTabTable[ti].client.minimized =
                        _M.altTabTable[ti].minimized
                    break
                end
            end
        end
...
function _M.clientOpacity()
    ...
    for i, data in pairs(_M.altTabTable) do data.client.opacity = opacity end
...
function _M.preview()
    ...
    local y = screen[mouse.screen].geometry.y +
                  (screen[mouse.screen].geometry.height - h - textboxHeight) / 2
    ...
    for i = 1, nLeft do
        table.insert(leftRightTab,
                     _M.altTabTable[#_M.altTabTable - nLeft + i].client)
    ...
    for i = 1, #leftRightTab do
        ...
        _M.preview_widgets[i].draw = function(preview_widget, preview_wbox, cr,
                                              width, height)
    ...
    for i = 1, #leftRightTab do preview_layout:add(_M.preview_widgets[i]) end
    preview_layout:add(spacer)
            ...
            if (key == release_key or key == "Escape") and event == "release" then
                ...
                if key == "Escape" then
                    for i = 1, #_M.altTabTable do
                        _M.altTabTable[i].client.opacity =
                            _M.altTabTable[i].opacity
                        _M.altTabTable[i].client.minimized =
                            _M.altTabTable[i].minimized
                    end
                else
