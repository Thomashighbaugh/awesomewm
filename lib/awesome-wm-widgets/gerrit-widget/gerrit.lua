...
local GET_CHANGES_CMD =
    [[bash -c "curl -s -X GET -n %s/a/changes/\\?q\\=%s | tail -n +2"]]
local GET_USER_CMD =
    [[bash -c "curl -s -X GET -n %s/accounts/%s/ | tail -n +2"]]
...
local function worker(args)
    ...
    local icon = args.icons or HOME_DIR ..
    ...
    gerrit_widget = wibox.widget {
        ...
        {id = "new_rev", widget = wibox.widget.textbox},
    ...
    local function get_name_by_user_id(user_id)
        if name_dict[user_id] == nil then name_dict[user_id] = {} end

        if name_dict[user_id].username == nil then
            name_dict[user_id].username = ''
            spawn.easy_async(string.format(GET_USER_CMD, host, user_id),
                ...
                if not gfs.file_readable(PATH_TO_AVATARS .. user_id) then
                    spawn.easy_async(string.format(DOWNLOAD_AVATAR_CMD,
                                                   PATH_TO_AVATARS .. user_id,
                                                   user.avatars[1].url))
                end
            end)
    ...
    local update_widget = function(widget, stdout, _, _, _)
        ...
        if current_number_of_reviews > previous_number_of_reviews then
            ...
            naughty.notify {
                ...
                text = reviews[1].project .. '\n' ..
                    ...
                    reviews[1].subject .. '\n',
                run = function()
                    spawn.with_shell("xdg-open https://" .. host .. '/' ..
                                         reviews[1]._number)
                end
            }
        end
        ...
        for _, review in ipairs(reviews) do
                ...
                {
                    {
                        ...
                        },
                        {
                            ...
                            {
                                text = '  ' ..
                                    get_name_by_user_id(review.owner._account_id),
                                widget = wibox.widget.textbox
                            },
            ...
            row:buttons(awful.util.table.join(
                    ...
                    spawn.with_shell("xdg-open " .. host .. '/' ..
                                         review._number)
                    popup.visible = false
                end), awful.button({}, 3, function()
                    spawn.with_shell("echo '" .. review._number ..
    ...
    gerrit_widget:buttons(awful.util.table.join(
                              awful.button({}, 1, function()
    ...
    watch(string.format(GET_CHANGES_CMD, host, query:gsub(" ", "+")), 10,
          update_widget, gerrit_widget)
