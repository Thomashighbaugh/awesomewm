...
w:setup{
    ...
    {
        {id = 'header', widget = wibox.widget.textbox},
        {id = 'src', widget = wibox.widget.textbox},
        {id = 'res', widget = wibox.widget.textbox},
        id = 'text',
...
local function translate(to_translate, lang, api_key)
    local urll = BASE_URL .. '?lang=' .. lang .. '&text=' ..
                     urlencode(to_translate) .. '&key=' .. api_key
    ...
    if (code == 200 and resp_json ~= nil) then
        ...
        w.left.text.src:set_markup('<b>' .. lang:sub(1, 2) ..
                                       '</b>: <span color="#FFFFFF"> ' ..
                                       to_translate .. '</span>')
        w.left.text.res:set_markup('<b>' .. lang:sub(4) ..
                                       '</b>: <span color="#FFFFFF"> ' .. resp ..
                                       '</span>')
        ...
        w:buttons(awful.util.table.join(awful.button({}, 1, function()
            awful.spawn.with_shell("echo '" .. resp ..
                                       "' | xclip -selection clipboard")
            w.visible = false
        end), awful.button({}, 3, function()
            awful.spawn.with_shell("echo '" .. to_translate ..
                                       "' | xclip -selection clipboard")
            w.visible = false
        end)))
...
input_widget:setup{layout = wibox.container.margin, prompt, left = 10}

local function show_translate_prompt(api_key)
    awful.placement.top(input_widget,
    ...
    awful.prompt.run {
        ...
        done_callback = function() input_widget.visible = false end
    }
end

return {show_translate_prompt = show_translate_prompt}
