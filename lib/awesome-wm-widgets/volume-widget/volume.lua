...
function volume:raise()
    volume:_cmd('amixer ' .. volume.device .. ' sset Master ' ..
                    tostring(volume.delta) .. '%+')
end
...
local function update_graphic(widget, stdout, _, _, _)
    ...
    if volume.display_notification then
        volume.notification.iconbox.image =
            PATH_TO_ICONS .. volume_icon_name .. ".svg"
...
local function worker(args)
    ...
    if volume_audio_controller == 'pulse' then volume.device = '-D pulse' end
