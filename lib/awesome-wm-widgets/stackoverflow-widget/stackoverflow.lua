...
local GET_QUESTIONS_CMD =
    [[bash -c "curl --compressed -s -X GET 'http://api.stackexchange.com/2.2/questions/no-answers?page=1&pagesize=%s&order=desc&sort=activity&tagged=%s&site=stackoverflow'"]]
...
local function worker(args)
    ...
    local update_widget = function(widget, stdout, stderr, _, _)
        ...
        for _, item in ipairs(result.items) do
            ...
            local row = wibox.widget {
                {
                    {
                        {text = item.title, widget = wibox.widget.textbox},
                        {
                            text = tags,
                            align = 'right',
                            widget = wibox.widget.textbox
    ...
    stackoverflow_widget:buttons(awful.util.table.join(
    ...
    watch(string.format(GET_QUESTIONS_CMD, limit, tagged), 300, update_widget,
