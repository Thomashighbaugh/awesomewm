...
function module.arrow_path2(cr, width, height, head_width, shaft_width,
                            shaft_length)
...
function module.arrow(width, sidesize, margin, bg_color, fg_color)
    local img = cairo.ImageSurface(cairo.Format.ARGB32, width + 2 * margin,
                                   width + 2 * margin)
...
local function refresh_dt(last_sec, last_usec, callback, delay)
    ...
    local dt = (tv.tv_sec * 1000000 + tv.tv_usec) -
                   (last_sec * 1000000 + last_usec)
    if dt < delay then callback() end
...
function module.double_click(callback, delay)
    ...
    local ds, du = 0, 0
    return function() ds, du = refresh_dt(ds, du, callback, delay) end
end
...
function module.get_ordered_screens()
    ...
    screens_inv = {}
    for k, v in ipairs(screens) do screens_inv[v] = k end

    return screens, screens_inv
end

capi.screen.connect_signal("added",
                           function() screens, screens_inv = nil, nil end)
