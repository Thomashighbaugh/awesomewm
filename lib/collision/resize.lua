...
local invert = {left = "right", right = "left", up = "down", down = "up"}
...
local function create_indicators()
    ...
    local bg = beautiful.collision_resize_bg or beautiful.bg_alternate or
                   "#ff0000"
            ...
            {
                {
                    ...
                    shape = shape.transform(shape.arrow):translate(tr, tr)
                        :rotate(angle):translate(-tr, -tr),
                    bg = fg,
...
function module.resize(mod, key, event, direction, is_swap, is_max)
    ...
    c:emit_signal("request::geometry", "mouse.resize",
                  r_ajust[direction](c, del))
