...
local layout = require("lib.collision.layout")
...
local function create_arrow(cr, x, y, width, height, direction)
    ...
    local s = shape.transform(shape.arrow):rotate_at(width / 4, height / 4,
                                                     math.pi / 2)

    if direction == 1 then s:rotate_at(width / 4, height / 4, math.pi) end
...
local function draw_shape(s, collection, current_idx, icon_f, y, text_height)
    ...
    -- Resize the wibox
    w.x, w.y, w.width, w.height = math.floor(geo.x), math.floor(
                                      y or (wa.y + wa.height) - margin - height),
                                  math.floor(geo.width), math.floor(height)
    ...
    -- Get the colors
    local white, bg = color("#FFFFFF"),
                      color(beautiful.collision_max_bg or
                                beautiful.menu_bg_normal or beautiful.bg_normal)

    local normal, focus = color(beautiful.collision_max_fg or
                                    beautiful.fg_normal),
    ...
    for k, v in ipairs(collection) do
        ...
        cr3:line_to(dx + margin + width - 2 * margin,
                    height - text_height - border)
        cr3:stroke()
...
function module.hide() w.visible = false end
...
local function client_icon(c, width, height)
    ...
    if geom.height * scale > height then scale = height / geom.height end
    local w, h = geom.width * scale, geom.height * scale

    -- Create a mask
    local s = shape.transform(rshape):translate((width - w) / 2,
                                                (height - h) / 2)
    s(cr, w, h, 10)
    ...
    if not icon then return img end
...
local function tag_icon(t, width, height)
    ...
    if icon_orig then
        local icon = cairo.ImageSurface(cairo.Format.ARGB32,
                                        icon_orig:get_width(),
                                        icon_orig:get_height())
...
local function change_tag(s, direction, is_swap)
    local s = capi.client.focus and capi.client.focus.screen or
                  capi.mouse.screen
    if not is_swap then
