...
local module = {
    _focus = require("lib.collision.focus"),
    _resize = require("lib.collision.resize"),
    _max = require("lib.collision.max"),
    _screen = require("lib.collision.screen"),
    mouse = require("lib.collision.mouse"),
    ...
    capi.keygrabber.run(function(mod, key, event)
        ...
        for k, v in pairs(keys) do
            if util.table.hasitem(v, key) then
                if event == "press" then
                    if not event_callback[current_mode](mod, key, event, k,
                                                        is_swap, is_max) then
                        return exit_loop()
                    end
                    return
        ...
        elseif key == "Control_L" or key == "Control_R" then
            is_max = event == "press"
            return #mod > 0 and awful.util.table.hasitem(mod, "Mod4") or
                       exit_loop()
...
function module.focus(direction, c, max)
    local screen = (c or
                       ((capi.client.focus and capi.client.focus.focusable) and
                           capi.client.focus or capi.mouse)).screen
    -- Useless when there is only 1 client tiled, incompatible with the "max_out" mode (in this case, focus floating mode)
    if awful.layout.get(screen) == awful.layout.suit.max and
        #awful.client.tiled(screen) > 1 and not max then
        current_mode = "max"
    ...
    glib.idle_add(glib.PRIORITY_DEFAULT_IDLE, function()
        for k, v in pairs(keys) do
            for _, key_name in ipairs(v) do
                aw[#aw + 1] = awful.key({"Mod4"}, key_name,
                                        function()
                    module.focus(k)
                end, {
                    description = "Change focus to the " .. key_name,
                    group = "Collision"
                })
                aw[#aw + 1] = awful.key({"Mod4", "Mod1"}, key_name,
                                        function()
                    module.resize(k)
                end, {
                    description = "Resize to the " .. key_name,
                    group = "Collision"
                })
                aw[#aw + 1] = awful.key({"Mod4", "Shift"}, key_name,
                                        function() module.move(k) end, {
                    description = "Move to the " .. key_name,
                    group = "Collision"
                })
                aw[#aw + 1] = awful.key({"Mod4", "Shift", "Control"}, key_name,
                                        function()
                    module.move(k, nil, true)
                end, {description = "", group = "Collision"})
                aw[#aw + 1] = awful.key({"Mod4", "Control"}, key_name,
                                        function()
                    module.focus(k, nil, true)
                end, {
                    description = "Change floating focus to the " .. key_name,
                    group = "Collision"
                })
                aw[#aw + 1] = awful.key({"Mod4", "Mod1", "Control"}, key_name,
                                        function()
                    module.screen(k)
                end, {
                    description = "Change screen to the " .. key_name,
                    group = "Collision"
                })
                aw[#aw + 1] = awful.key({"Mod1", "Shift", "Control", "Mod4"},
                    ...
                    description = "Move tag screen to the " .. key_name,
                    group = "Collision"
                })
                if k == "left" or k == "right" then -- Conflict with my text editor, so I say no
                    aw[#aw + 1] = awful.key({"Mod1", "Control"}, key_name,
                                            function()
                        module.tag(k, nil, true)
                    end, {
                        description = "Select tag to the " .. key_name,
                        group = "Collision"
                    })
                    aw[#aw + 1] = awful.key({"Mod1", "Shift", "Control"},
                                            key_name, function()
                        module.tag(k, true, true)
                    end, {
                        description = "Move tag to the " .. key_name,
                        group = "Collision"
                    })
                end
            end
