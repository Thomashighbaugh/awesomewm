...
function module.get_geometry(tag)
    ...
    for k, c in ipairs(tag:clients()) do
        ...
        if not c.minimized then
            ...
            if (not floating) and (not l == awful.layout.suit.floating) then
                ...
                if c == focus then focus_wrap = cls[#cls] end
            else
    ...
    if not focus_wrap then focus_wrap = cls[1] end
...
function module.draw(tag, cr, width, height)
    ...
    local r, g, b = util.get_rgb(beautiful.collision_max_fg or
                                     beautiful.fg_normal)
    ...
    for c, ll in ipairs({l, l2}) do
        for c, geom in pairs(ll) do
            shape.transform(lshape):translate(
                geom.x * ratio * w_stretch + margin, geom.y * ratio + margin)(
                cr, geom.width * ratio * w_stretch - margin * 2,
                geom.height * ratio - margin * 2, radius)
            cr:close_path()
