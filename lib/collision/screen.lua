...
local mouse = require("lib.collision.mouse")
local util = require("lib.collision.util")
...
local function current_screen(focus)
    return (not focus) and capi.mouse.screen or
...
local function fit(self, _, width, height)
    ...
    return math.min(geo.x + geo.width, min),
    math.min(geo.y + geo.height, min) + tm
end
...
local function create_wibox(s)
    ...
    local bg = beautiful.collision_screen_bg or beautiful.bg_alternate or
    "#ff0000"
    local fg = beautiful.collision_screen_fg or beautiful.fg_normal or "#0000ff"
    local bg_focus =
    beautiful.collision_screen_bg_focus or beautiful.bg_urgent or "#ff0000"
    local fg_focus =
    beautiful.collision_screen_fg_focus or beautiful.fg_urgent or "#ff0000"
    ...
    w:setup{
        {
            ...
            {
                nil,
                {screen = s, padding = padding, widget = new_constrained_text},
                nil,
...
local function init_wiboxes()
    ...
    awful.screen.connect_for_each_screen(function(s) create_wibox(s) end)
...
local function select_screen(scr_index, move, old_screen)
    ...
    if c and c.valid and c:isvisible() then
        ...
        if last_clients_coords[scr_index] and
            last_clients_coords[scr_index].client == c then
            capi.mouse.coords(last_clients_coords[scr_index])
        else
            capi.mouse.coords({
                x = geom.x + geom.width / 2,
                y = geom.y + geom.height / 2 + 55
            })
        end
    else
...
local function in_rect(c, point)
    ...
    return
    (geo.x < point.x and geo.y < point.y and geo.x + geo.width > point.x and
    geo.y + geo.height > point.y)
end
...
local function save_cursor_position()
    ...
    if c and in_rect(c, coords) then
        last_clients_coords[c.screen] = {client = c, x = coords.x, y = coords.y}
    else
...
local function next_screen(ss, dir, move)
    ...
    if type(scr_index) == "screen" then scr_index = scr_index.index end
    if dir == "left" then
...
function module.display(_, dir, move)
    if #wiboxes == 0 then init_wiboxes() end
...
local function highlight_screen(ss)
    ...
    if pss ~= ss then
        ...
        local bg = beautiful.collision_screen_bg or beautiful.bg_alternate or
        "#ff0000"
        local fg = beautiful.collision_screen_fg or beautiful.fg_normal or
        "#0000ff"
        local bg_focus = beautiful.collision_screen_bg_focus or
        beautiful.bg_urgent or "#ff0000"
        local fg_focus = beautiful.collision_screen_fg_focus or
        beautiful.fg_urgent or "#ff0000"
...
function module.hide()
    ...
    for s = 1, capi.screen.count() do wiboxes[s].visible = false end
mouse.hide()
end

local function show()
    for s = 1, capi.screen.count() do wiboxes[s].visible = true end
end
...
awful.screen.connect_for_each_screen(function(s)
    if next(wiboxes) then create_wibox(s) end
end)

capi.client.connect_signal("focus", function(c) last_clients[c.screen] = c end)
