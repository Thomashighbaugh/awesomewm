...
local col_utils = require("lib.collision.util")
...
local function init()
    ...
    local bg = beautiful.collision_focus_bg or beautiful.bg_alternate or
    "#ff0000"
    local fg = beautiful.collision_focus_fg or beautiful.fg_normal or "#0000ff"
    local bg_focus =
    beautiful.collision_focus_bg_center or beautiful.bg_urgent or "#ff0000"
    ...
    for k, v in ipairs({"up", "right", "down", "left", "center"}) do
        wiboxes[v] = wibox {height = sh, width = sw, ontop = true}
        ...
        wiboxes[v]:setup{
            v ~= "center" and {
                {
                    ...
                    shape = shape.transform(col_utils.arrow_path2):rotate_at(x,
                        y,
                        (k -
                        1) *
                        (2 *
            ...
            } or {widget = wibox.widget.imagebox},
            bg = r_bg,
            shape = r_shape,
...
local function float_move_max(dir, c)
    return ({
        ...
        right = {
            x = capi.screen[c.screen].workarea.width +
            capi.screen[c.screen].workarea.x - c:geometry().width
        },
        up = {y = capi.screen[c.screen].workarea.y},
        down = {
            y = capi.screen[c.screen].workarea.y +
            capi.screen[c.screen].workarea.height - c:geometry().height
        }})[dir]
    end
    
    local function floating_clients()
        local ret = {}
        for v in util.table.iterate(client.visible(),
            function(c) return c.floating end) do
ret[#ret + 1] = v
        end
        return ret
    end
    ...
    local function bydirection(dir, c, swap, max)
        if not c then c = emulate_client(capi.mouse.screen) end
        ...
        if swap and float then
            ...
            local cltbl, geomtbl, scrs, roundr, roundl =
            max and floating_clients() or client.tiled(), {}, {}, {}, {}
            ...
            if swap ~= true then
                ...
                if target then
                    ...
                    if cl and cl.is_screen then
                        ...
                        if not old_src or capi.client.focus.screen ~=
                            capi.screen[old_src] then
                ...
                if target then
                    ...
                    local other =
                    cltbl[((not cltbl[target] and #cltbl == 1) and 1 or target)]
                    if capi.screen[other.screen] == capi.screen[c.screen] or
                        col_utils.settings.swap_across_screen then
    ...
    function module._global_bydirection_key(mod, key, event, direction, is_swap,
    is_max)
...
function module.display(mod, key, event, direction, is_swap, is_max)
    ...
    for i, cl in ipairs(cltbl) do geomtbl[i] = cl:geometry() end

    -- Sometime, there is no focussed clients
    if not c then c = geomtbl[1] or cltbl[1] end
    
    -- If there is still no accessible clients, there is nothing to display
    if not c then return end
    
    display_wiboxes(cltbl, geomtbl, c.floating or alayout.get(c.screen) ==
    alayout.suit.floating, is_swap, c)
end
