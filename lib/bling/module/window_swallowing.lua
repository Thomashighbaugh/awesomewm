...
local dont_swallow_classname_list = beautiful.dont_swallow_classname_list or
                                        {"firefox", "Gimp"}
local activate_dont_swallow_filter = beautiful.dont_swallow_filter_activated or
                                         true
...
local function check_if_swallow(c)
    if not activate_dont_swallow_filter then return true end
    for _, classname in ipairs(dont_swallow_classname_list) do
        if classname == c.class then return false end
    end
    return true
end
...
local function manage_clientspawn(c)
    ...
    local handle = io.popen([[pstree -T -p -a -s ]] .. tostring(c.pid) ..
                                [[ | sed '2q;d' | grep -o '[0-9]*$' | tr -d '\n']])
    local parent_pid = handle:read("*a")
    handle:close()

    if (tostring(parent_pid) == tostring(parent_client.pid)) and
        check_if_swallow(c) then
