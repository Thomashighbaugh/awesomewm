...
local delayed_call = (type(timer) ~= 'table' and
...
local revelation = {
    ...
    -- for comparison.
    match = {exact = aw_rules.match, any = aw_rules.match_any},
    property_to_watch = {
        maximized = false,
        minimized = false,
        fullscreen = false,
    ...
    border_color = beautiful.revelation_border_color or beautiful.border_focus or
        "#DCDCCC",
    border_width = beautiful.revelation_border_width or beautiful.border_width or
        2,
    hintsize = (type(beautiful.xresources) == 'table' and
        beautiful.xresources.apply_dpi(beautiful.revelation_hintsize or 50) or
        60)
}
...
local function match_clients(rule, _clients, t, is_excluded)
    ...
    local mf =
        is_excluded and function(c, _rule) return not mfc(c, _rule) end or mfc
    local flt
    ...
    for _, c in pairs(_clients) do
        if mf(c, rule) then
            ...
            toggle_tag_func(t, c)
            if c:isvisible() then table.insert(clients, c) end
        end
    end
...
function revelation.expose(args)
    ...
    for scr = 1, capi.screen.count() do
        t[scr] = awful.tag.new({revelation.tag_name}, scr,
                               awful.layout.suit.fair)[1]
        zt[scr] = awful.tag.new({revelation.tag_name .. "_zoom"}, scr,
                                awful.layout.suit.fair)[1]
...
function revelation.restore(t, zt)
    ...
    for i, j in pairs(hintindex) do hintbox[i].visible = false end
end
...
local function hintbox_pos(char)
    ...
    hintbox[char].x = math.floor(geom.x + geom.width / 2 - revelation.hintsize /
                                     2)
...
function revelation.expose_callback(t, zt, clientlist)
    ...
    capi.keygrabber.run(function(mod, key, event)
        ...
        if awful.util.table.hasitem(mod, "Shift") then
            ...
            elseif zoomedClient ~= nil then
                ...
                toggle_tag_func(zt[zoomedClient.screen.index or
                                    zoomedClient.screen], zoomedClient)
    ...
    capi.mousegrabber.run(function(mouse)
        ...
        elseif mouse.buttons[2] == false and pressedMiddle == true then
            pressedMiddle = false
            for key, _ in pairs(hintindex) do hintbox_pos(key) end
...
setmetatable(revelation,
             {__call = function(_, ...) return revelation.expose(...) end})
