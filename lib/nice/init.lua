    ...
    local buttons = {
        awful.button({}, _private.mb_move, function()
            ...
            if clicks == 2 then
                ...
                if shade_enabled then _private.shade_roll_down(c) end
                c:activate{context = "titlebar", action = "mouse_move"}
            end
            -- Start a timer to clear the click count
            gtimer_weak_start_new(delta, function() clicks = 0 end)
        end), awful.button({}, _private.mb_contextmenu, function()
        ...
        end), awful.button({}, _private.mb_resize, function()
            c:activate{context = "mouse_click", action = "mouse_resize"}
        end)
    }

    if _private.window_shade_enabled then
        buttons[#buttons + 1] = awful.button({}, _private.mb_win_shade_rollup,
                                             function()
            _private.shade_roll_up(c)
        end)
        buttons[#buttons + 1] = awful.button({}, _private.mb_win_shade_rolldown,
                                             function()
            _private.shade_roll_down(c)
        end)
    end
