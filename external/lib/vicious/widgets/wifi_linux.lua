...
local function parser(stdout, stderr, exitreason, exitcode)
	...
	winfo["{rate}"] = tonumber(stdout:match("Bit Rate[=:]%s?([%d%.]+)") or 0)
	winfo["{freq}"] = tonumber(stdout:match("Frequency[=:]%s?([%d%.]+)") or 0) * 1000
	winfo["{txpw}"] = tonumber(stdout:match("Tx%-Power[=:](%d+)") or 0)
	winfo["{link}"] = tonumber(stdout:match("Link Quality[=:](%d+)") or 0)
	winfo["{linp}"] = winfo["{link}"] ~= 0 and math.floor(winfo["{link}"] / 0.7 + 0.5) or 0
