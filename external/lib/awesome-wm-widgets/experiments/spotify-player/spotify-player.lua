...
local function worker(args)
	...
	local function get_artwork(track_id, url)
		if ((url ~= nil or url ~= "") and not gfs.file_readable("/tmp/" .. track_id)) then
			spawn.easy_async("touch /tmp/" .. track_id, function()
				spawn.easy_async(
					"curl -L -s --show-error --create-dirs -o /tmp/" .. track_id .. " " .. url,
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		local track_id, length, art_url, album, album_artist, artist, auto_rating, disc_number, title, track_number, url = string.match(
				stdout,
				"trackid|(.*)\nlength|(.*)\nartUrl|(.*)\nalbum|(.*)\nalbumArtist|(.*)\nartist|(.*)\nautoRating|(.*)\ndiscNumber|(.*)\ntitle|(.*)\ntrackNumber|(.*)\nurl|(.*)"
			)
		...
		play_button:buttons(awful.util.table.join(awful.button({}, 1, function()
		...
		prev_button:connect_signal("button::press", function(c)
			c:set_bg(beautiful.bg_focus)
		end)
		prev_button:connect_signal("button::release", function(c)
	...
	spotify_player:buttons(awful.util.table.join(awful.button({}, 1, function()
...
return setmetatable(spotify_player, { __call = function(_, ...)
	return worker(...)
end })
