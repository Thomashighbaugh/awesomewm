...
local function create_button_image(name, is_focused, event, is_on)
	...
	if not _private[key_color] then
		...
		button_color = (event == "hover")
			and colors.lighten(button_color, 25)
			or (event == "press")
			and colors.darken(button_color, 25)
			or button_color
...
local function create_titlebar_button(c, name, button_callback, property)
	...
	button_img.buttons = awful.button({}, nice.MB_LEFT, function()
		event = "press"
		update()
	end, function()
...
local function get_titlebar_mouse_bindings(c)
	...
	local buttons = {
		...
		awful.button({}, _private.mb_resize, function()
			c:activate({ context = "mouse_click", action = "mouse_resize" })
		end),
	}

	if _private.window_shade_enabled then
		buttons[#buttons + 1] = awful.button({}, _private.mb_win_shade_rollup, function()
			_private.shade_roll_up(c)
		end)
		buttons[#buttons + 1] = awful.button({}, _private.mb_win_shade_rolldown, function()
			_private.shade_roll_down(c)
		end)
	end
	return buttons
