...
local session = require("external.lib.poppin.session")

local apps = {}
local manage = function()
end
...
function pop(name, command, ...)
    ...
    local app = apps[name]
    if app == nil then
        initClient(name, command, args.position, args.size, args.properties, args.callback)
