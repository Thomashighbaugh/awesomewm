...
local decorate_titlebar = function(c, bg, size)
    ...
    local setupTable = {
        ...
        {buttons = buttons, layout = wibox.layout.flex[layout],             widget = awful.titlebar.widget.titlewidget(c), button
    }

    awful.titlebar(c, {position = titlebar_pos, bg = beautiful.background, size = size or titlebar_size}):setup(
        setupTable
    )
end
