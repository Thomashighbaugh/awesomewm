--[[ 
	 _______ ___ ___ _______ _______      _______ ______ ______ _______ _______ _______    __              
|    ___|   |   |_     _|_     _|    |     __|      |   __ \    ___|    ___|    |  |  |  |.--.--.---.-.
|    ___|-     -|_|   |_  |   |      |__     |   ---|      <    ___|    ___|       |__|  ||  |  |  _  |
|_______|___|___|_______| |___|      |_______|______|___|__|_______|_______|__|____|__|__||_____|___._|
     ...
     Description:
       The dialog screen where the user can log out, reboot, etc
       implemented in lua for consistency and because I can
--]]
...
exit_widget.start_watch = function()
    ...
    fullname="$(getent passwd `whoami` | cut -d ':' -f 5 | cut -d ',' -f 1 | tr -d "\n")"
        ...
        function(stdout)
            ...
            exit_widget.content.greeter =
                'Leaving so soon, ' .. first_name:sub(1, 1):upper() .. first_name:sub(2) .. '?!'
...
exit_widget.poweroff_command = function()
    awful.spawn.with_shell('sudo nas-umount ; umount-modules ; sudo poweroff')
end
-- exit_widget.reboot_command = function() awful.spawn.with_shell('sudo reboot') end
exit_widget.reboot_command = function()
    awful.spawn.with_shell('sudo nas-umount ; umount-modules ; sudo reboot')
end

exit_widget.build_button = function(args, icon, function_table, name)
    local clickable_icon, image_to_change = clickable_image(args, icon, function_table)
    local button_text =
        wibox.widget {
        text = name,
...
exit_widget.build_leave_widget = function()
    ...
    local button_args = {}
    local poweroff_button =
        exit_widget.build_button(
        button_args,
        icons.power,
        function()
            exit_widget.poweroff_command()
        end,
        'Shutdown'
    )
    local reboot_button =
