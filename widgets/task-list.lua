...
local function list_update(w, buttons, label, data, objects)
    ...
    for i, o in ipairs(objects) do
        ...
        if cache then
            ...
            cb = clickable_container(wibox.container.margin(wibox.widget.imagebox(ICON_DIR .. "close.svg"), dpi(4), dpi(4), dpi(4), dpi(4)))
            cb.shape = gears.shape.circle
            cbm = wibox.container.margin(cb, dpi(4), dpi(4), dpi(4), dpi(4)) -- close button
            ...
            ibm = wibox.container.margin(ib, dpi(1), dpi(1), dpi(1), dpi(1)) --app icon
        ...
        if text == nil or text == '' then
            ...
            if (text_only:len() > 8) then
                text = text:gsub('>(.-)<', '>' .. text_only:sub(1, 5) .. '...<')
