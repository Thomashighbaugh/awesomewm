...
local wall_dir = config_dir .. 'theme/'						-- Wallpaper directory
local default_wall_name = 'Vice-Kali-Yantra.png'						-- Default wallpaper
...
local locker = function(s)
	...
	local caps_text = wibox.widget {
		...
		font = 'SF TransRobotics Italic 10',
		align = 'center',
		valign = 'center',
		opacity = 0.0,
		widget = wibox.widget.textbox
	...
	local time = wibox.widget.textclock(
		'<span font="SF TransRobotics 56">%H:%M</span>',
		1
	)

	local time_shadow = wibox.widget.textclock(
		'<span foreground="#00000066" font="SF TransRobotics 56">%H:%M</span>',
		1
	)

	local wanted_text = wibox.widget {
		markup = 'INTRUDER ALERT!',
		font   = 'Future Forces 3D12',
		align  = 'center',
		valign = 'center',
		widget = wibox.widget.textbox
	}
	...
	local red = beautiful.xcolor1
local green = beautiful.xcolor2
	local yellow = beautiful.xcolor3
	local blue = beautiful.xcolor4
