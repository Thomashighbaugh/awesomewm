...
blur_widget.start_watch = function()
	...
	awesome.connect_signal(
		...
		sed -i 's/.*strength = .*/    strength = ]] ..
					blur_widget.value / 50 * 10 .. [[;/g' "${picom_dir}"
		]],
...
blur_widget.build_dashboard = function(args)
	local slider =
		...
		nil,
		{
			...
			bar_height = dpi(2),
			bar_color = beautiful.xbackground .. '99',
			bar_active_color = beautiful.color7,
			handle_color = beautiful.xforeground,
			handle_shape = gears.shape.circle,
			handle_width = dpi(15),
			handle_border_color = beautiful.xbackground .. '99',
	...
	local buttons =
		...
		awful.button(
			...
			nil,
			function()
				awesome.emit_signal('widget::blur:set', 100)
			end
		)
	)

	local widget_button,
		widget_icon = clickable_image(args, icons.widget.effects, buttons)

	local blur_dashboard =
		wibox.widget {
		{
			{
				widget_button,
	...
	awesome.connect_signal(
		...
		function()
			slider.blur_slider:set_value(blur_widget.value)
		end
	)
...
blur_widget.build_toggler = function(args)
	local dashboard_text =
		wibox.widget {
		text = 'Blur Effects',
		font = 'SF Mono Semibold 11',
		align = 'left',
		widget = wibox.widget.textbox
	}
	local dashboard_toggle_button,
		dashboard_toggle_icon =
		clickable_image(
		args,
		icons.toggled_off,
		function()
