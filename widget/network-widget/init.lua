  ...
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      ...
      elseif string.match(nmcli_output, "connected to ") then
        awful.spawn.easy_async_with_shell( -- 'iw dev ' .. apps.default.wifi_interface .. ' link',
          "iw dev ",
          function(stdout)
...
network_widget.start_watch = function()
  ...
  awful.spawn.easy_async_with_shell(
    ...
    function(stdout)
      wifi_interface = stdout
    end
  )
  awful.spawn.easy_async_with_shell(
    "ls /sys/class/net | grep en",
    function(stdout)
      eth_interface = stdout
    end
  )
...
network_widget.build = function(args)
  ...
  local table =
    ...
    awful.button(
      ...
      nil,
      function()
        awful.spawn.with_shell(apps.default.network_manager, false)
      end
    ),
    awful.button(
      {},
      ...
      nil,
      function()
        network_widget.toggle_vpn()
      end
    )
  )
  local widget_button, imagebox = clickable_image(args, icons.widget.network.no_network, table)
...
network_widget.build_dashboard = function(args)
  ...
  local build_dasboard_element = function(init_image, icon_action, toggle_icon_action)
    local args = {callback = callbacks.zoom, tooltip = false}
    local dashboard_button, dashboard_icon = clickable_image(args, init_image, icon_action)
    ...
    local dashboard_toggle_button, dashboard_toggle_icon = clickable_image(args, icons.toggled_off, toggle_icon_action)

    local dashboard_widget =
      wibox.widget {
      {
        {
          {
  ...
  local dashboard_wifi =
    build_dasboard_element(
    icons.widget.network.wifi_off,
    function()
      awful.spawn(apps.default.terminal .. " -e nmtui")
    end,
    function()
      network_widget.toggle_wifi()
    end
  )
  local dashboard_eth =
    build_dasboard_element(
  ...
  local update_icon = function()
    ...
    if wifi_connection then
      ...
      dashboard_wifi.text:set_text(essid .. ": " .. tostring(wifi_strength) .. " %")
