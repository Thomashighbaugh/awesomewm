...
local create_clock = function(s)
	...
	if not military_mode then
		clock_format = '<span font="Play Bold 11">%I:%M %p</span>'
	else
		clock_format = '<span font="Play Bold 11">%H:%M</span>'
	end
	...
	s.clock_tooltip = awful.tooltip
	{
		...
		timer_function = function()
			...
			local date_str = 'Today is ' .. os.date('%A') ..' the ' ..
			'<span color="#00caff">' .. day .. ordinal .. 
			' of ' .. month ..  '</span> in the year ' .. os.date('%Y') .. '. \n'
			return date_str
		end,
	}
